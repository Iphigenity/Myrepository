...
<SCRIPT language=javascript>
	...
	function Choose(cusid,custName) {
		//获取添加联系人页面的window对象引用
		var win = window.opener;
		//获取document对象
		var doc = win.document;
		//向指定id元素添加cusid和custName
		doc.getElementById("cusid").value=cusid;
							...
							<TBODY>
								<TR>
									...
									<FORM id="pageForm" name="customerForm"
										...
										<input type="hidden" name="choose"  value="<s:property value="#parameters.choose" />" />
										...
										<TABLE id=grid
											...
											<TBODY>
												...
												<TR 		
													...
													<TD>
													<s:property value="#cust.custLevel.dict_item_name" />
													</TD>
													<TD>
													<s:property value="#cust.custSource.dict_item_name" />
													</TD>
													<TD>
													<s:property value="#cust.linkMen" />
													</TD>
													
													<TD>
													<s:if test="#parameters.choose==null">
													<a href="${pageContext.request.contextPath }/CustomerAction_Edit?cusid=<s:property value="#cust.cusid" />">修改</a>
													&nbsp;&nbsp;
													<a href="${pageContext.request.contextPath }/CustomerAction_delete?cusid=<s:property value="#cust.cusid" />">删除</a>
													</s:if>
													<s:else>
													<input type="button" value="选择"  onclick="Choose('<s:property value="#cust.cusid" />','<s:property value="#cust.custName" />')">
													</s:else>
													</TD>
												</TR>
												</s:iterator>
